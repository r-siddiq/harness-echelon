...
CLAUDE.md is Architect-owned policy. The Orchestrator and all delegated agents treat it as read-only during a directive unless the Architect explicitly requests a rules change. The Orchestrator's framework-maintenance write authority is limited to PLAN.md, `.tasks.json`, and root `log.md`.
...
- Delegated agents never write PLAN.md, `.tasks.json`, root `log.md`, or CLAUDE.md. The Orchestrator may maintain its shared control files while implementation runs but never touches a writer's product-file write set.
