...
- Agent executes → orchestrator waits until `COMPLETE`, `BLOCKED:`, or `PAUSE:`; while the agent runs, the orchestrator may prepare only non-overlapping next steps, verification, or integration work without duplicating work or touching the agent's write set
- When agent returns → verify results → append `progressEntries` to .tasks.json → mark completed → archive the task and its exact task-specific progress to root `log.md` → remove them from .tasks.json and recompute `phaseHealth` → repeat
...
All active, non-completed orchestrator execution state is stored in `.tasks.json` — it is the authoritative active-state schema. The full schema is defined there; the Orchestrator reads it directly and delegates only task-scoped fragments. Completed-task history is stored in root `log.md`.
...
|--------|-----------------|---------------------|
| `COMPLETE` | → `completed` (Phase B) or → `spec_ready` (Phase A) | Verify against success criteria → append `progressEntries` → for verified Phase B completion, archive to `log.md` and remove active records → next task |
...
7. After verification, archive each completed task and its exact matching task-specific progress to root `log.md` before removing them from `.tasks.json`.
...
**2. Execution** — The orchestration loop: decompose approved phases into .tasks.json tasks → dispatch Phase A research → synthesize findings → dispatch Phase B implement → verify → archive verified completion to root log.md → remove it from active state → repeat. This is the primary operational mode.
...
6. For a verified completed task, archive its full object and exact matching task-specific progress to root `log.md` before removing those records from `.tasks.json`
...
- **Active-ledger threshold:** If `progressEntries` exceeds 15 entries during an active phase, compact only unscoped historical phase telemetry when safe; never remove progress for non-completed tasks. Completed-task progress leaves active state only with its task during the root `log.md` archive step.
...
4. **On completion:** update .tasks.json with `completed` state and append `progressEntries`; after verification, archive the task and its exact matching progress to root `log.md` before removing them from active state
