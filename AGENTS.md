...
- Delegated agents never write PLAN.md, `.tasks.json`, root `log.md`, or AGENTS.md. The Orchestrator may maintain the shared control files while implementation runs but never touches the active writer's product-file write set.
...
| Orchestrator | `All tools` | Sole runtime writer of PLAN.md, `.tasks.json`, and root `log.md`; AGENTS.md remains read-only without explicit Architect direction; never touches an active writer's product-file write set |
